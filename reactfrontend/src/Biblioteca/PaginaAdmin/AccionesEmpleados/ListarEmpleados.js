...
import { generarUUID, peticionDelete, peticionGetAuth, mostrarAlertaCorrecta, mostrarAlertaErronea } from '../../FuncionesAuxiliares/Funciones';
        ...
        if(option.apellidos === ""){
            return (
                <tr className='EmpleadoTablaApartado' key={generarUUID()}>
                    <td colSpan="3">
                        Tu empresa no dispone de empleados.
                    </td>
                    <td colSpan="3" className="alturaDefinidaTabla">
                        <Link title='Ver Información Del Empleado' to='/altaEmpleado' className="botonPadPequeño botonInfoCliente anyadirTurnoBoton linkSignInLogin" id="signIn">
                            AÑADIR EMPLEADOS
                        </Link>
                    </td>
                </tr>
            ...
            <tr className='EmpleadoTablaApartado' key={generarUUID()}>
                ...
                <td>
                    <button type='button' className='sinBorde' to='/modificarEmpleado' onClick={modificar}>
                        <img title='Modificar Empleado' className='imagenFotoGestionUsuarios' id={option.id} src={require('../../../img/modify-foto.png')} alt="imagen Foto Modificar"/>
                    </button>
                    <button type='button' className='sinBorde' onClick={borrarEmpleado}>
                        <img title='Borrar Empleado' className='imagenFotoGestionUsuarios' id={option.id} src={require('../../../img/delete-foto.png')} alt="imagen Foto Borrar"/>
                    </button>
